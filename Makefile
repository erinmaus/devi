...
$(BUILD_DIR)/$(ZLIB_FILE)-build: $(BUILD_DIR)/$(ZLIB_FILE)/CMakeLists.txt
	cmake -B$(BUILD_DIR)/$(ZLIB_FILE)-build -H$(BUILD_DIR)/$(ZLIB_FILE) $(CMAKE_OPTS) -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$(BUILD_DIR) -DBUILD_SHARED_LIBS=OFF
...
$(BUILD_DIR)/lib/libz.$(LIB_EXT): $(BUILD_DIR)/$(ZLIB_FILE)-build
	...
	rm -rf $(BUILD_DIR)/lib/libz*$(SHARED_LIB_EXT)
...
$(BUILD_DIR)/$(LIBPNG_FILE)-build-x86_64: $(BUILD_DIR)/$(LIBPNG_FILE)/CMakeLists.txt $(BUILD_DIR)/lib/libz.$(LIB_EXT)
	cmake -B$(BUILD_DIR)/$(LIBPNG_FILE)-build-x86_64 -H$(BUILD_DIR)/$(LIBPNG_FILE) -DCMAKE_OSX_ARCHITECTURES="x86_64" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$(BUILD_DIR) -DBUILD_SHARED_LIBS=OFF

$(BUILD_DIR)/$(LIBPNG_FILE)-build-arm64: $(BUILD_DIR)/$(LIBPNG_FILE)/CMakeLists.txt $(BUILD_DIR)/lib/libz.$(LIB_EXT)
	cmake -B$(BUILD_DIR)/$(LIBPNG_FILE)-build-arm64 -H$(BUILD_DIR)/$(LIBPNG_FILE) -DCMAKE_OSX_ARCHITECTURES="arm64" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$(BUILD_DIR) -DBUILD_SHARED_LIBS=OFF
...
$(BUILD_DIR)/$(LIBPNG_FILE)-build/$(LIBPNG_LIB).$(LIB_EXT): $(BUILD_DIR)/$(LIBPNG_FILE)/CMakeLists.txt $(BUILD_DIR)/lib/libz.$(LIB_EXT)
	cmake -B$(BUILD_DIR)/$(LIBPNG_FILE)-build -H$(BUILD_DIR)/$(LIBPNG_FILE) $(CMAKE_OPTS) -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$(BUILD_DIR) -DBUILD_SHARED_LIBS=OFF
...
$(BUILD_DIR)/lib/$(LIBPNG_LIB).$(LIB_EXT): $(BUILD_DIR)/$(LIBPNG_FILE)-build
endif
...
ifeq ($(shell uname),Darwin)
ifeq ($(PLATFORM),IOS)
	cd $(BUILD_DIR)/$(GIFLIB_FILE) && make CC="$(CC)" LD="$(LD)" CFLAGS="$(CFLAGS)" LDFLAGS="$(LDFLAGS)" clean libgif.$(LIB_EXT)
else
...
$(BUILD_DIR)/$(BIN): $(OBJ)
	$(CXX) $(LDFLAGS) $(DEVI_LDFLGS) -llua51 -lz -lpng16 -lgif $^ -o $@
