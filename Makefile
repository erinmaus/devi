...
$(BUILD_DIR)/$(LIBPNG_FILE)-build/$(LIBPNG_LIB).$(LIB_EXT): $(BUILD_DIR)/$(LIBPNG_FILE)/CMakeLists.txt $(BUILD_DIR)/lib/libz.$(LIB_EXT)
	cmake -B$(BUILD_DIR)/$(LIBPNG_FILE)-build -H$(BUILD_DIR)/$(LIBPNG_FILE) $(CMAKE_OPTS) -DCMAKE_BUILD_TYPE=Release -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_PREFIX=$(BUILD_DIR) -DBUILD_SHARED_LIBS=OFF
